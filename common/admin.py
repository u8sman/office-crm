...
from common.models import Department, Reminder, TheFile, UserProfile
from common.site import reminderadmin, userprofileadmin
...
class ReminderAdmin(ModelAdmin):
    ...
    search_fields = ("subject", "description", "owner__username")
...
class TheFileAdmin(ModelAdmin):
    ...
    readonly_fields = ("to_object",)  # <-- only this, no file_url
...
class FileInline(GenericStackedInline):
    ...
    def has_add_permission(self, request, obj):
        # whoever can change the parent object
        # should have permission to add inline
    ...
    def has_delete_permission(self, request, obj=None):
        # whoever can change the parent object
        # should have permission to delete inline
        return self.has_change_permission(request, obj)
    ...
    def clarify_permission(request, obj):
        if hasattr(obj, "owner"):
            if obj.owner == request.user or not obj.owner:
                if any(
                    (
                        hasattr(obj, "REVIEWED") and obj.stage == obj.REVIEWED,
